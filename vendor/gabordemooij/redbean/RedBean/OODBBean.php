...
class RedBean_OODBBean implements IteratorAggregate, ArrayAccess, Countable
{
	 ...
	 * @param RedBean_BeanHelper $beanhelper bean helper to obtain a toolbox and a models
	 *
	 * @return void
	 */
	...
	/**
	 * Sends the call to the registered models.
	 *
	...
	public function __call( $method, $args )
	{
		if ( !isset( $this->__info['models'] ) ) {
			...
			$this->__info['models'] = $model;
		}
		if ( !method_exists( $this->__info['models'], $method ) ) {
			return NULL;
		}

		return call_user_func_array( array( $this->__info['models'], $method ), $args );
	}

	/**
	 * Implementation of __toString Method
	 * Routes call to Model. If the models implements a __toString() method this
	 * method will be called and the result will be returned. In case of an 
	 * echo-statement this result will be printed. If the models does not
